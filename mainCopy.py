...
import dictionaryOfCommands1 as lists
...
class Bot:
    ...
    def radio(self):
        ...
        # url = "https://music.yandex.ru/users/yamusic-daily/playlists/128579782?from=alice&mob=0&play=1"
        # https://radio.yandex.ru/mood/beautiful?from=alice&mob=0&play=1

        webbrowser.open(url)

    def translator(self):
        Speech("Сейчас переведу")
        url = 'https://translate.yandex.ru/'
        webbrowser.open(url)

    def vk(self):
        Speech("Вам кто-то написал!")
        url = 'https://vk.com'
        webbrowser.open(url)

    def youTube(self):
        Speech("Посмотрим какое-либо видео")
    ...
    def movie(self):
        Speech("Давайте посмотрим фильм")
